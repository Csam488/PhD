...
while read Chr Start Stop Other; do
	...
	echo -e "${Chr}\t${Start}\t${Stop}\t${Other}\t$(bedtools intersect -f 0.7 -F 0.7 -u -a ${annotation_dir}/Data/DUP.bed -b ${output}/Temp.bed | sort -k4,4n | cut -f 4 | cat - ${output}/0.tmp | head -n 1)" >> ${output}/ALL.unsort.unann.bed
done < ${output}/DUP.bed
while read Chr Start Stop Other; do
	echo -e "${Chr}\t${Start}\t${Stop}" > ${output}/Temp.bed
	echo -e "${Chr}\t${Start}\t${Stop}\t${Other}\t$(bedtools intersect -f 0.7 -F 0.7 -u -a ${annotation_dir}/Data/DEL.bed -b ${output}/Temp.bed | sort -k4,4n | cut -f 4 | cat - ${output}/0.tmp | head -n 1)" >> ${output}/ALL.unsort.unann.bed
done < ${output}/DEL.bed
while read Chr Start Stop Other; do
	echo -e "${Chr}\t${Start}\t${Stop}" > ${output}/Temp.bed
	echo -e "${Chr}\t${Start}\t${Stop}\t${Other}\t$(bedtools intersect -f 0.7 -F 0.7 -u -a ${annotation_dir}/Data/INV.bed -b ${output}/Temp.bed | sort -k4,4n | cut -f 4 | cat - ${output}/0.tmp | head -n 1)" >> ${output}/ALL.unsort.unann.bed
done < ${output}/INV.bed
while read Chr Start Stop Chr2 Start2 Stop2 Name Other; do
	echo -e "${Chr}\t$(( ${Start} - 500))\t$(( ${Stop} + 500))" > ${output}/Temp.bed
	bedtools intersect -f 0.7 -F 0.7 -u -a ${annotation_dir}/Data/TRLX.bedpe -b ${output}/Temp.bed | awk -v T='\t' -F '\t' '{print $4T$5T$6T$1T$2T$3T$7}' > ${output}/Temp.Match.bed
