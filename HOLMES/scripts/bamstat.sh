...
if $SORT; then
	...
	sambamba sort -n -m 4GB -p \
	-o ${BAMFILE}.tmp_nsort.bam ${BAMFILE}.nosecondary.tmp_bamstat.bam
	${liWGS_SV}/bamstat/bamstat -i ${BAMFILE}.tmp_nsort.bam -d 7 -b >>${OUTDIR}/logs/bamstat.log
	...
	${liWGS_SV}/bamstat/bamstat -i ${BAMFILE}.nosecondary.tmp_bamstat.bam -d 7 -b >>${OUTDIR}/logs/bamstat.log
...
do
	echo -e "STATUS [$(date)]: Running readpaircluster on ${svtype}..."
	sort -k2,2 -k5,5 -k3n,3n ${svtype}_pairs.txt > ${svtype}_pairs.sorted.txt;
	#${liWGS_SV}/readpaircluster/readPairCluster -d $DISTANCE -s $SIZE -q $MAPQ -r ${svtype}_pairs.sorted.txt > ${svtype}_clusters_d${DISTANCE}_q${MAPQ}_s${SIZE}.txt #&
	python ${liWGS_SV}/readpaircluster/rpc.py -d $DISTANCE -s $SIZE -q $MAPQ ${svtype}_pairs.sorted.txt ${svtype}_clusters_d${DISTANCE}_q${MAPQ}_s${SIZE}.txt
	echo -e "STATUS [$(date)]: Finished readpaircluster on ${svtype}..."
done
