...
#  cat <( head -n1 ${cov_data} ) <( awk -v chr=chr${contig} '{ if ($1==chr) print }' ${cov_data} ) > ./cnMOPS_chrsplit/${contig}/${group}.rawCov.chr${contig}.bed
...
data_frame2GRanges <- function(df, keepColumns = TRUE, ignoreStrand = FALSE) {
    ...
    if(keepColumns) {
        dt <- as(df[, setdiff(names(df), c("Chr", "Start", "Stop", "strand"))],
                     "DataFrame")
...
if(rebinsize>1){
  cov_data <- rebin(read.table(path.to.matrix,header=T,sep="\t",stringsAsFactors=F),rebinsize)
} else {
  cov_data <- read.table(path.to.matrix,header=T,sep="\t",stringsAsFactors=F)
}
cov_data[2:ncol(cov_data)] <- apply(cov_data[2:ncol(cov_data)],2,function(vals){return(as.numeric(as.character(vals)))})
colnames(cov_data)[c(2,3)] = c("Start","Stop")
cov_data[is.na(cov_data)] <- 0
print('Converting to GRanges')
cov_data <- data_frame2GRanges(cov_data)
print('Running cn.mops')
#Runs cn.mops
res <- cn.mops(cov_data, parallel = 4)
print('Finishing cn.mops')
