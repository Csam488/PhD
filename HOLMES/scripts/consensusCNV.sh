...
samples_list=$5         #FIX including sample list variable (not in params)
...
echo -e "STATUS [$(date)]: Converting Classifier Files"
case ${cnvtype} in
  del)
...
echo -e "STATUS [$(date)]: Converting cnMOPS Files"
...
echo -e "STATUS [$(date)]: Calculating Max Insert Size"
max_insert=$( fgrep -v "#" ${OUTDIR}/QC/cohort/${COHORT_ID}.QC.metrics | cut -f9 | sort -nrk1,1 | head -n1 )

#Get number of samples in cohort
echo -e "STATUS [$(date)]: Calculating Number of Samples"
nsamp_all=$( wc -l < ${samples_list} )

#Get sex indexes; used for genotyping X and Y
echo -e "STATUS [$(date)]: Getting Sex Indexes and Counts"
...
echo -e "STATUS [$(date)]: Screening Reciprocal Overlaps"
while read chr start end cID samples; do
  ...
  bedtools intersect -wa -wb -r -f 0.5 -b <( echo -e "${chr}\t${start}\t${end}" ) -a ${cnMOPS} | fgrep -wf ${samp_clust} > ${overlap}
...
echo -e "STATUS [$(date)]: Removing intersected IDs and printing calls"
...
echo -e "STATUS [$(date)]: Assigning temp IDs"
...
echo -e "STATUS [$(date)]: For Autosomes"
#Genotype autosomes
...
for contig in X; do
  ...
  for sex in M F; do
    if [ ${cnvtype} == "del" ]; then
      #cd ${WRKDIR}/consensusCNV/${cnvtype}_genotyping/plots/${contig}/${sex}
      #module load R/3.1.0
...
echo -e "STATUS [$(date)]: Merging Genotypes"
...
echo -e "STATUS [$(date)]: Overlapping group A"
...
while read chr start end eID count_union samples_union cluster_ID count_cluster samples_cluster cnMOPS_ID count_cnMOPS samples_cnMOPS cnMOPS_consensus; do
  bedtools coverage -a ${CNV_BLACKLIST} -b <( echo -e "${chr}\t${start}\t${end}") | awk '{ if ($10 >= 0.3) print $0 }' > ${blk_check}
...
echo -e "STATUS [$(date)]: Overlapping group B"
...
echo -e "STATUS [$(date)]: Overlapping group C"
...
echo -e "STATUS [$(date)]: Applying Size Filters"
#Size Filters
