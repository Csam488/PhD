...
echo -e "***BUILDING EXON LIST***"
for contig in $( seq 1 22 ) X Y; do
  bash ${liWGS_SV}/scripts/build_exons.sh hg19 ${contig} 1 300000000 ${retTMP}/${contig}_exons.bed
done
cat ${retTMP}/*_exons.bed | sed 's/^chr//g' | sed -e 's/^X/23/g' -e 's/^Y/24/g' | sort -nk1,1 -nk2,2 | sed -e 's/^23/X/g' -e 's/^24/Y/g' > ${retTMP}/all_exons.bed

#Prepares outfile
echo -e "eID\tTotal_Mappings\tMappings_in_Exons\tPct_Exonic\tGene(s)" > ${OUTDIR}/candidate_retrotransposons.list

#Deletion
echo -e "***CHECKING DELETIONS***"
while read eID; do
  ...
  num_ovr=$( bedtools intersect -u -a ${TMPPOS} -b ${retTMP}/all_exons.bed | wc -l )
  if [[ ${num_ovr} -ge ${min_rds} ]]; then
    gene_ovr=$( bedtools intersect -wao -a ${TMPPOS} -b ${retTMP}/all_exons.bed | cut -f7 | sort | uniq | grep -ve '^$' | paste -s | sed 's/\t/,/g' )
...
#Translocation
echo -e "***CHECKING TRANSLOCATIONS***"
while read eID; do
