...
WORKING=`mktemp -d ${TMPDIR}/tmp.XXXXXXXX`
samp_list=`mktemp`
clust=`mktemp`
remove=`mktemp`
prefinal=`mktemp`
...
while read ID bed; do
  paste <( awk -v OFS="\t" -v contig=${contig} -v ID=${ID} '{ if ($1==contig) print $1, $2, $3, ID, ID"_"NR }' ${bed} ) <( awk -v contig=${contig} '{ if ($1==contig) print $0 }' ${bed} | cut --complement -f1-3  | sed 's/\t/\//g' ) > ${WORKING}/${ID}.bed
done < ${in_list}

#cat & sort master bed
cat ${WORKING}/*bed | sed -e 's/^X/23/g' -e 's/^Y/24/g' | sort -nk1,1 -nk2,2 -nk3,3 > ${WORKING}/all.bed
...
while read chr start end ID eID info; do
  ...
  if [ $( fgrep -w ${eID} ${remove} | wc -l ) -eq 0 ]; then
    awk -v chr=${chr} -v start=${start} -v end=${end} -v dist=${cluster_dist} '{ if ($1==chr && $2>=(start-dist) && $2<=(start+dist) && $3>=(end-dist) && $3<=(end+dist)) print $0 }' ${WORKING}/all.bed | fgrep -wvf ${remove} > ${clust}
    if [ $( cat ${clust} | wc -l ) -gt 1 ]; then
      ...
      until [ ${rep} -eq 0 ]; do
        awk -v chr=${chr} -v midA=${midA} -v midB=${midB} -v dist=${cluster_dist} '{ if ($1==chr && $2>=(midA-dist) && $2<=(midA+dist) && $3>=(midB-dist) && $3<=(midB+dist)) print $0 }' ${WORKING}/all.bed | fgrep -wvf <( cut -f5 ${clust} ) | fgrep -wvf ${remove} | fgrep -wvf <( cut -f4 ${clust} ) > ${add}
...
done < ${WORKING}/all.bed | paste - - - - - - - - - > ${prefinal}
...
#Clean up
rm -rf ${WORKING}
rm ${samp_list} ${clust} ${remove} ${prefinal} ${add}