...
while read -u 3 ID bam sex; do
  echo -e "STATUS [$(date)]: Collecting Metrics for ${ID}..."
  if ! [ -e ${OUTDIR}/QC/sample/${ID}/${ID}.insert_size_metrics ]; then
    sambamba view -h -f bam -F 'not secondary_alignment' ${WRKDIR}/${ID}/${ID}.bam | java -Djava.io.tmpdir=$TMPDIR/tmp -jar ${PICARD} CollectMultipleMetrics I=/dev/stdin O=${OUTDIR}/QC/sample/${ID}/${ID} AS=true R=${REF} VALIDATION_STRINGENCY=SILENT PROGRAM=null PROGRAM=CollectAlignmentSummaryMetrics PROGRAM=CollectInsertSizeMetrics 2>> ${OUTDIR}/logs/input_QC.log
  fi
  if ! [ -e ${OUTDIR}/QC/sample/${ID}/${ID}.flagstat ]; then
    sambamba view -h -f bam -F 'not secondary_alignment' ${WRKDIR}/${ID}/${ID}.bam | samtools flagstat /dev/stdin > ${OUTDIR}/QC/sample/${ID}/${ID}.flagstat 2>> ${OUTDIR}/logs/input_QC.log
  fi
  if ! [ -e ${OUTDIR}/QC/sample/${ID}/${ID}.stats ]; then
    sambamba view -h -f bam -F 'not secondary_alignment' ${WRKDIR}/${ID}/${ID}.bam | bamtools stats -insert > ${OUTDIR}/QC/sample/${ID}/${ID}.stats 2>> ${OUTDIR}/logs/input_QC.log
  fi
  if ! [ -e ${OUTDIR}/QC/sample/${ID}/${ID}.wgs ]; then
    java -Djava.io.tmpdir=$TMPDIR/tmp -jar ${PICARD} CollectWgsMetrics I=${WRKDIR}/${ID}/${ID}.bam O=${OUTDIR}/QC/sample/${ID}/${ID}.wgs R=${REF} VALIDATION_STRINGENCY=SILENT 2>> ${OUTDIR}/logs/input_QC.log
  fi
  if ! [ -e ${OUTDIR}/QC/sample/${ID}/${ID}.complexity ]; then
    java -Djava.io.tmpdir=$TMPDIR/tmp -jar ${PICARD} EstimateLibraryComplexity I=${WRKDIR}/${ID}/${ID}.bam O=${OUTDIR}/QC/sample/${ID}/${ID}.complexity VALIDATION_STRINGENCY=SILENT 2>> ${OUTDIR}/logs/input_QC.log
  fi
  echo -e "STATUS [$(date)]: Executing WGScheckDosage..."
  bash ${liWGS_SV}/scripts/WGScheckDosage.sh ${WRKDIR}/${ID}/${ID}.bam ${ID} h37 both ${OUTDIR}/QC/sample/${ID}
  echo -e "STATUS [$(date)]: Executing sexCheck..."
  bash ${liWGS_SV}/scripts/sexCheck.sh ${ID} ${WRKDIR}/${ID}/${ID}.bam ${params}
  echo -e "STATUS [$(date)]: Executing chrCopyCount..."
  bash ${liWGS_SV}/scripts/chrCopyCount.sh ${ID} ${WRKDIR}/${ID}/${ID}.bam ${params}
done 3< ${samples_list}
...
#  echo -e "STATUS [$(date)]: For ${ID}..."
#  bash ${liWGS_SV}/scripts/WGScheckDosage.sh ${WRKDIR}/${ID}/${ID}.bam ${ID} h37 both ${OUTDIR}/QC/sample/${ID}/
#  echo -e "STATUS [$(date)]: Completed ${ID}..."
#done < ${samples_list}
...
echo -e "STATUS [$(date)]: Parse anuploidy check..."
...
echo -e "STATUS [$(date)]: Plot aneuploidy check..."
if ! [ -d ${OUTDIR}/plots/AneuploidyCheck ]; then
  mkdir ${OUTDIR}/plots/AneuploidyCheck
fi
Rscript ${liWGS_SV}/scripts/plotAneuCheck.R ${OUTDIR}/QC/cohort/${COHORT_ID}.aneuploidyCheck.fractions.txt ${OUTDIR}/QC/cohort/${COHORT_ID}.aneuploidyCheck.copies.txt ${OUTDIR}/plots/AneuploidyCheck/${COHORT_ID}.aneu.fractions.pdf ${OUTDIR}/plots/AneuploidyCheck/${COHORT_ID}.aneu.copies.pdf >> ${OUTDIR}/logs/input_QC.log

#Run dosage Z-score
echo -e "STATUS [$(date)]: Run dosage Z-scores..."
ID=$( head -n1 ${samples_list} | cut -f1 )
...
echo -e "STATUS [$(date)]: Collecting summary for each sample..."
...
echo -e "$(date)" > ${OUTDIR}/checkpoints/module1.done
