...
if [ -e ${WRKDIR}/cnMOPS.input ]; then
  rm ${WRKDIR}/cnMOPS.input
fi
while read ID bam sex; do
  echo -e "${WRKDIR}/${ID}/${ID}.bam\t${ID}" >> ${WRKDIR}/cnMOPS.input
done < ${samples_list}
bash ${liWGS_SV}/scripts/binCov.sh ${WRKDIR}/cnMOPS.input ${DICT} physical 1000 ${COHORT_ID} ${WRKDIR}/iCov
...
echo -e "$(date)" > ${OUTDIR}/checkpoints/module2.done