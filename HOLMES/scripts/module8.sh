...
echo -e "STATUS [$(date)]: Writing Things"
...
echo -e "STATUS [$(date)]: Simple Inversions"
...
echo -e "STATUS [$(date)]: Simple Translocations"
...
while read chrA minA maxA chrB minB maxB cluster class; do
  observations=$( fgrep -w ${cluster} ${WRKDIR}/raw_clusters/${COHORT_ID}.transloc.events.bedpe | cut -f19 )
  samples=$( fgrep -w ${cluster} ${WRKDIR}/raw_clusters/${COHORT_ID}.transloc.events.bedpe | cut -f20 | tr -d "[]" )
  if [ ${class} == "Insertion" ]; then #ERROR WITH UNDEIFED CLASS?
...
echo -e "STATUS [$(date)]: Complex Events"
...
echo -e "STATUS [$(date)]: Unresolved Events"
...
echo -e "$(date)" > ${OUTDIR}/checkpoints/module8.done